...
import com.test.NormalMode.BarChart;
...
public class BarChartUI extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Button linechart_createButton = (Button) findViewById(R.id.barchart_createButton);
		Button linechart_addButton = (Button) findViewById(R.id.barchart_addButton);
		Button linechart_clearList = (Button) findViewById(R.id.barchart_clearList);
		final EditText linechart_inputName = (EditText) findViewById(R.id.barchart_inputName);
		final EditText linechart_fileName = (EditText) findViewById(R.id.barchart_fileName);
		
		HowInputNum = 8;

		// 下面几个变量用于动态生成输入数据表格
		final TableLayout linechart_table_list = (TableLayout) findViewById(R.id.barchart_table_list);
		final TableRow[] tableRow = new TableRow[10];
		final TextView[] viewText1 = new TextView[10];
		final TextView[] viewText2 = new TextView[10];

		// 动态生成输入框
		final TableLayout inputnum_list = (TableLayout) findViewById(R.id.barchart_inputnum_list);
		...
		linechart_addButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					ju.connServerForResult("http://192.168.68.1:8887/charts/bar_submit.php?username="+gp.getPara(v.getContext(),"username")+"&filename="+gp.getPara(v.getContext(),"filename")+"&name="+linechart_fileName.getText()+"&"+value_all);
		...
		linechart_createButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				GetPara gp=new GetPara();
				SetPara sp=new SetPara();
				ju2.connServerForResult("http://192.168.68.1:8887/charts/bar.php?username="+gp.getPara(v.getContext(),"username")+"&filename="+gp.getPara(v.getContext(),"filename"));
				...
				try {
					...
					IChart[] mCharts = new IChart[] { new BarChart() };
					intent = mCharts[0].execute(BarChartUI.this);
	...
	private void showErrorAlert(CharSequence message) {
		new AlertDialog.Builder(BarChartUI.this).setTitle("			错误！").setMessage(
